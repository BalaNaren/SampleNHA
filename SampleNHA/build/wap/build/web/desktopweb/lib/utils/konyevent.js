...
(function() {
    ...
    var _handleKeyBoardClick = function $KE_handleKeyBoardClick(evt, model) {
        ...
        if(evt.type === 'keydown') {
            ...
            if(_handleSkinEvent.mouseOverModel === model) {
            ...
            if(_handleSkinEvent.mouseOverModel === model && model.hoverSkin) {
    ...
    var _handleSkinEvent = {
        ...
        dealWithActiveStateSkin: function(evt) {
            ...
            _handleSkinEvent.activeTargetNode = _handleSkinEvent.targetRow ? _handleSkinEvent.targetRow
                                                : evt.target;
            ...
            if(model) {
                ...
                if(['focusin'].indexOf(evt.type) >= 0) {
                    if(model instanceof kony.ui.GroupWidget &&
                        ...
                        _handleSkinEvent.dealWithGroupWidgetActiveStateSkin(model);
                    } else if((_handleSkinEvent.mouseOverModel !== model)
                        || (!model.focusSkin && !model.hoverSkin)) {
                        _handleSkinEvent.setSkinRecursively(model, 'active');
                    }

                    _handleSkinEvent.focusInModel = model;
                } else if(['focusout'].indexOf(evt.type) >= 0) {
                    _handleSkinEvent.resetSkinRecursively(model, 'active');
                    _handleSkinEvent.focusInModel = null;
                }
            }
        },
        ...
        dealWithFocusSkin: function(evt) {
            ...
            _handleSkinEvent.targetRow = evt.target;

            if(!$KD.getAttr(evt.target, 'kr') && !$KD.hasAttr(evt.target, 'kw')) {
                _handleSkinEvent.targetRow = $KD.parent(evt.target);
            }
            ...
            if(model) {
                ...
                if(['mousedown', 'touchstart'].indexOf(evt.type) >= 0) {
                    ...
                    if(model instanceof kony.ui.GroupWidget) {
                        _handleSkinEvent.activeTargetNode = _handleSkinEvent.targetRow;
                        isWidgetHovered = model._kwebfw_.hSkinNode ?
                                          (model._kwebfw_.hSkinNode === _handleSkinEvent.targetRow) : false;
                    } else {
                        isWidgetHovered = model.hoverSkin ? true : false;
                    }

                    if(_handleSkinEvent.focusInModel === model) {
                        if(isWidgetHovered) {
                            _handleSkinEvent.resetSkinRecursively(model, 'active');
                        } else {
                            _handleSkinEvent.setSkinRecursively(model, 'active');
                    ...
                    _handleSkinEvent.targetRow = '';
                }
            }
        },

        dealWithGroupWidgetActiveStateSkin: function(model) {
            var $K = kony.$kwebfw$, $KD = $K.dom, target = _handleSkinEvent.activeTargetNode,
                mouseDownOnModel = model.focusSkin ? model._kwebfw_.fSkinNode: null,
                mouseOverOnModel = model.hoverSkin ? model._kwebfw_.hSkinNode : null;

            if(!$KD.hasAttr(target, 'kw')
            && ($KD.getAttr(target, 'kr') !== 'option')) {
                target = $KD.parent(target);
                _handleSkinEvent.activeTargetNode = target;
            }

            if(target !== (mouseDownOnModel || mouseOverOnModel)) {
                _handleSkinEvent.setSkinRecursively(model, 'active');
            }
        },
        ...
        dealWithGroupWidgetHoverSkin: function(model) {
            ...
            || ['CheckBoxGroup', 'RadioButtonGroup'].indexOf($KW.name(model)) >= 0) {
                ...
                if(!option && !model._kwebfw_.hSkinNode && model.activeStateSkin) {
                ...
                if(option && model._kwebfw_.hSkinNode !== option) {
                    ...
                    if(_handleSkinEvent.activeTargetNode !== model._kwebfw_.hSkinNode && hSkin) {
                        _handleSkinEvent.setSkin(model, aSkin, 'active');
                    }
                }

                if($KU.is(model, 'widget', 'ListBox') || (option && model._kwebfw_.hSkinNode !== option)) {
                    hSkin && _handleSkinEvent.setSkin(model, hSkin, 'hover');
                ...
                if(relatedModel === model) {
                    ...
                    if(relatedModel) {
                        if($KU.is(relatedModel, 'widget', 'Calendar') && relatedModel._kwebfw_.hSkinNode) {
                            _handleSkinEvent.dealWithCalendarHoverSkin(relatedModel);//handle for picker
                                ...
                                if(_handleSkinEvent.focusInModel === relatedModel && relatedModel.hoverSkin) {
                    ...
                    if(($KU.is(model, 'widget', 'Calendar') && model._kwebfw_.picker === target)) {
                       _handleSkinEvent.dealWithCalendarHoverSkin(model);//handle for picker
                            ...
                            if(_handleSkinEvent.focusInModel === model && model.hoverSkin) {
                                _handleSkinEvent.resetSkinRecursively(model, 'active');
                            }
        ...
        setSkin: function(model, skin, type) {
            ...
            if(el) {
                ...
                && model.viewType === constants.LISTBOX_VIEW_TYPE_EDITVIEW)) {
                    ...
                    if(type === 'active') {
                        el = _handleSkinEvent.targetRow ? _handleSkinEvent.targetRow : _handleSkinEvent.activeTargetNode;
                    } else {
                        el = _handleSkinEvent.target;
                    }
    ...
    var _registerSystemEvents = function $KE_registerSystemEvents() {
        var $K = kony.$kwebfw$, $KD = $K.dom, $KU = $K.utils, events = [
                'blur',
                'error',
                'focus',
                'hashchange',
