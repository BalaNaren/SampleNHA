...
//pollyfill ends
(function() {
    var VERSION = '9.4.3.v202112271838', BUILD = 'debug', ECMAENABLE = "@ECMAENABLE",
        PUBLISH = false, PLATFORMBUILT ='desktopweb', UWPAPIS = (/true/i).test('-- UWPAPIS --'),
    ...
    function loadFiles(head, sources, sync, onsuccess, onerror) {
        ...
        for(s=0; s<slen; s++) {
            if(typeof sync === 'boolean') {
                loadScript(head,  loadPath+sources[s], sync, function() {counter++;}, function() {failed = true;});
            } else {
                loadStyle(head,  loadPath+sources[s], function() {counter++;}, function() {failed = true;});
            }
        }
